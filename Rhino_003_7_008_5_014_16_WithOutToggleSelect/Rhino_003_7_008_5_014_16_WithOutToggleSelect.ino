
// included spi and sd lib
// only nozzle 5 active 
// Dividing the functions 
// Removed pulseIn Function for PWM_2
// Removed Toggle1 Physical Switch Logic
// Incorporated Toggle 1 physical switch
// Updated to 5 Modules with light indication
// Updated to 2 Modules 
// Single Module Test :- 28-01-2025
// Implemented RPM Filter for single module :- 05-02-2025
// extended filter 
// blockage function

/*
-------------MOTOR MODEL-------------------
Base Motor :- JGB 37 - 520 - 12V - 60R  (60RPM, 100RPM)
Purchase Link :- 

Base Motor :- SPG 30E - 60K (75 RPM )
Purchase Link :- 

----------------RMCS2303--------------------
Purchase Link :- 

-----------------MEGA---------------------
Purchase Link :- 

-----------------------SLAVE ID SELECTION----------------------------
  (4     2    1) - Binary Select

   J1   J2   J3  (from led on board)
   0    0    0  - to ignore hardware selection
   0    0    1  - n1_id 
   0    1    0  - n2_id
   0    1    1  - n3_id
   1    0    0  - n4_id
   1    0    1  - n5_id
   1    1    0  - n6_id
   1    1    1  - n7_id

---------------------MOTOR CONTROLE COMMANDS------------------------- 
  MOTOR CONTROL 
  1)  rmcs.Enable_Digital_Mode(n1_id,1) - Wil enable motor in digital speed control mode. 0-fwd,1-reverse direction. 
  2)  rmcs.Speed(n1_id,speed) - Will Set the Speed(RPM)
  3)  rmcs.Brake_Motor(n1_id,0) - Brake motor. 0-fwd,1-reverse direction. 
  4)  rmcs.Speed_Feedback(n1_id) - RPM Feed Back (incorrect for magnetic encoder)
  5)  rmcs.Disable_Digital_Mode(n1_id,0) - Disable motor in digital speed control mode. 0-fwd,1-reverse direction. 

  PID COMMANDS 
  1)  rmcs.WRITE_PARAMETER(n1_id,INP_CONTROL_MODE,PP_gain,PI_gain,VF_gain,LPR,acceleration,speed) - To write parameters to motor driver
  2)  rmcs.READ_PARAMETER(n1_id) - To read parameters from motor driver

  Digital Speed Control Mode

0101 (257) Enable motor in CW
0100 (256) Disable motor in CW
0104 (260) Brake in CW
0109 (265) Enable motor in CCW
0108 (264) Disable motor in CCW
010C (268) Brake in CCW


-------------------------CONNECTIONS----------------------------
Motor Driver to Encoder Motor  (n1_id) -> ref SLAVE ID SELECTION
   GND     ->   GND 
   +5VDC   ->   VCC
   ENC_B   ->   C2
   ENC_A   ->   C1
   HALL_U  ->   No Connection 
   W       ->   No Connection
   MOTOR - ->   M2 
   MOTOR + ->   M1
   VDD     ->   12v(Power Source) 
   GND     ->   GND(Power Source)

Motor Driver To Mega
   1-GND   ->   GND 
   2-RXD   ->   TX1 (D18)
   3-TXD   ->   RX1 (D19)

   ENC_A   ->   D2  (For Pulse Feedback Alarm)

Interrupt pins in mega - 2,3,21,20,A4,A5

Buzzer To Mega 
emty ->  GND 
 +   ->  D11
*/
//------------ SD Card --------------
#include <SD.h>
#include <SPI.h>
//-------------------------------


#include <FastLED.h>
#define LED_PIN     2
#define NUM_LEDS    20
#include<RMCS2303drive.h>
RMCS2303 rmcs;  //object for class RMCS2303
CRGB leds[NUM_LEDS];

/*
//------------ SD Card --------------
File myFile;
int pinCS = 53; // Pin 10 on Arduino Uno
//--------------
*/
//------------ SD Card --------------
int pinCS = 53; // Pin 10 on Arduino Uno
//--------------



byte n1_id=1;     //**********  NEED TO CHECK *****************
byte n2_id=2;
byte n3_id=3;
byte n4_id=4;
byte n5_id=5; 

int INP_CONTROL_MODE=257;
int PP_gain=32;//32
int PI_gain=16;//16
int VF_gain=32;//32

int LPR = 11;
int acceleration=5000;//5000
int speed=1000; //8000

int min_pwm = 1051;  //**********  NEED TO CHECK *****************
int max_pwm = 1951;  //**********  NEED TO CHECK *****************
int min_rpm = 0;     //**********  NEED TO CHECK *****************
int max_rpm = 6500; //(JGB 37 - 520 - 12V - 66RPM (66 RPM)) 
const int inputPwmSignalPin = 9;
const int inputPwmSignalPin_2 = 5; 
const int rpmCutOff = 1250;

int dt2 = 100; //operational delay 
volatile int ToggleState = 0;

const int relayPin = 10;
int rpm_diff = 25;
volatile int blocked = 0;

double FBSLL;
double FBSUL;
double FBSLL2;
double FBSUL2;
double FBSLL3;
double FBSUL3;
double FBSLL4;
double FBSUL4;
double FBSLL5;
double FBSUL5;

double GR = 0.0111; // 66RPM Motor
double CurrentRPM;
double CurrentRPM2;
double CurrentRPM3;
double CurrentRPM4;
double CurrentRPM5;

double errorRPM;
double errorRPM2;
double errorRPM3;
double errorRPM4;
double errorRPM5;

double setRPM;

int BlockageCounter = 0;
int BlockageCounter2 = 0;
int BlockageCounter3 = 0;
int BlockageCounter4 = 0;
int BlockageCounter5 = 0;

int BlockMaxLimit = 2;
int dt = 200; //loop time in ms

int config1;
int config2;
int config3;
int config4;
int config5;
int config6;
int config7;
int config8;
int config9;
int config10;
int config11;
int config12;
int config13;
int config14;
int config15;
int config16;
int config17;

int config1_pwm = 1101;
int config2_pwm = 1151;
int config3_pwm = 1201;
int config4_pwm = 1251;
int config5_pwm = 1301;
int config6_pwm = 1351;
int config7_pwm = 1401;
int config8_pwm = 1451;
int config9_pwm = 1501;
int config10_pwm = 1551;
int config11_pwm = 1601;
int config12_pwm = 1651;
int config13_pwm = 1701;
int config14_pwm = 1751;
int config15_pwm = 1801;
int config16_pwm = 1851;
int config17_pwm = 1901;

int config1_pwm_min = 1076;
int config2_pwm_min = 1126;
int config3_pwm_min = 1176;
int config4_pwm_min = 1226;
int config5_pwm_min = 1276;
int config6_pwm_min = 1323;
int config7_pwm_min = 1376;
int config8_pwm_min = 1426;
int config9_pwm_min = 1476;
int config10_pwm_min = 1526;
int config11_pwm_min = 1576;
int config12_pwm_min = 1626;
int config13_pwm_min = 1676;
int config14_pwm_min = 1726;
int config15_pwm_min = 1776;
int config16_pwm_min = 1826;
int config17_pwm_min = 1876;

int config1_pwm_max = 1126;
int config2_pwm_max = 1176;
int config3_pwm_max = 1226;
int config4_pwm_max = 1276;
int config5_pwm_max = 1326;
int config6_pwm_max = 1376;
int config7_pwm_max = 1426;
int config8_pwm_max = 1476;
int config9_pwm_max = 1526;
int config10_pwm_max = 1576;
int config11_pwm_max = 1626;
int config12_pwm_max = 1676;
int config13_pwm_max = 1726;
int config14_pwm_max = 1776;
int config15_pwm_max = 1826;
int config16_pwm_max = 1876;
int config17_pwm_max = 1926;

int read_input_pwm;
int read_input_pwm_2;
int n5_feed_back;
unsigned long curr_time_ms;

void setup()
{
/*
 //------------ SD Card ---------------
   pinMode(pinCS, OUTPUT);

   // SD Card Initialization
  if (SD.begin())
  {
    Serial.println("SD card is ready to use.");
  } else
  {
    Serial.println("SD card initialization failed");
    //return;
  }
  
  // Create/Open file 
  myFile = SD.open("test.txt", FILE_WRITE);
  
  // if the file opened okay, write to it:
  if (myFile) {
    Serial.println("Writing to file...");
    // Write to file
    myFile.println("DSR V2 Data Logging Events");
    // myFile.close(); // close the file
    // Serial.println("Done.");
  }
  // if the file didn't open, print an error:
  else {
    Serial.println("error opening test.txt");
  }
  //-----------------------------------------

*/

//------------ SD Card ---------------
   pinMode(pinCS, OUTPUT);

   // SD Card Initialization
  if (SD.begin(pinCS))
  {
    Serial.println("SD card is ready to use.");
  } else
  {
    Serial.println("SD card initialization failed");
    //return;
  }
  
  // Create/Open file 
  File dataFile = SD.open("datalog.txt", FILE_WRITE);

  
  // if the file opened okay, write to it:
  if (dataFile) {
    Serial.println("Writing to file...");
    // Write to file
    dataFile.println("******************************** DSR V2 Data Logging Events ****************************************");
    dataFile.close();
    // myFile.close(); // close the file
    // Serial.println("Done.");
  }
  // if the file didn't open, print an error:
  else {
    Serial.println("error opening test.txt");
  }
  //-----------------------------------------


   pinMode(relayPin,OUTPUT);
   digitalWrite(relayPin,LOW);
   FastLED.addLeds<WS2812, LED_PIN, GRB>(leds, NUM_LEDS);

   rmcs.Serial_selection(0);       //0-Hardware serial,1-Software serial
   rmcs.Serial0(9600); // Set Serial0 Baud    
   rmcs.begin(&Serial3,9600);    //mega2560:Serial1,Serial2,Serial3 and set baudrate.
  // rmcs.WRITE_PARAMETER(n1_id,INP_CONTROL_MODE,PP_gain,PI_gain,VF_gain,LPR,acceleration,speed); //Write parameters to drive
  // rmcs.WRITE_PARAMETER(n2_id,INP_CONTROL_MODE,PP_gain,PI_gain,VF_gain,LPR,acceleration,speed);
  // rmcs.WRITE_PARAMETER(n3_id,INP_CONTROL_MODE,PP_gain,PI_gain,VF_gain,LPR,acceleration,speed);
  // rmcs.WRITE_PARAMETER(n4_id,INP_CONTROL_MODE,PP_gain,PI_gain,VF_gain,LPR,acceleration,speed);
   rmcs.WRITE_PARAMETER(n5_id,INP_CONTROL_MODE,PP_gain,PI_gain,VF_gain,LPR,acceleration,speed);

  // rmcs.Disable_Digital_Mode(n1_id,0);
  // rmcs.Disable_Digital_Mode(n2_id,0);
  // rmcs.Disable_Digital_Mode(n3_id,0);
  // rmcs.Disable_Digital_Mode(n4_id,0);
   rmcs.Disable_Digital_Mode(n5_id,0);
}

void speed_select()
{
  File dataFile = SD.open("datalog.txt", FILE_WRITE);
   //read_input_pwm = pulseIn(inputPwmSignalPin,HIGH); // for flight test
   
   read_input_pwm = 1000; //for test bench
   speed = map(read_input_pwm,min_pwm,max_pwm,min_rpm,max_rpm);
   speed = constrain(speed, min_rpm, max_rpm); // Sets Minimum and maximum limits
   //Serial.print("Calculated RPM : ");
   //Serial.println(speed);
   dataFile.print(curr_time_ms);
   dataFile.print(" : Aux1 PWM = ");
   dataFile.println(read_input_pwm);

   dataFile.print(curr_time_ms);
   dataFile.print(" : Computed Base Motor RPM = ");
   dataFile.println(speed);

   setRPM = speed*GR;
   dataFile.print(curr_time_ms);
   dataFile.print(" : Set RPM = ");
   dataFile.println(setRPM);

  dataFile.close();    
}

void configuration_select()
{
  File dataFile = SD.open("datalog.txt", FILE_WRITE);
  //read_input_pwm_2 = pulseIn(inputPwmSignalPin_2,HIGH);
    read_input_pwm_2 = 1101;
   //Serial.println(read_input_pwm_2);

   dataFile.print(curr_time_ms);
   dataFile.print(" : Aux2 PWM = ");
   dataFile.println(read_input_pwm_2);
  
   if(read_input_pwm_2 <= config1_pwm_min)
   {
    config1 = 1;
    config2 = 0; 
    config3 = 0;
    config4 = 0;
    config5 = 0;
    config6 = 0;
    config7 = 0;
    config8 = 0;
    config9 = 0;
    config10 = 0;
    config11 = 0;
    config12 = 0;
    config13 = 0;
    config14 = 0;
    config15 = 0;
    config16 = 0;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 1 = ACTIVE");

   }
   else if (read_input_pwm_2 >= config2_pwm_min && read_input_pwm_2 <= config2_pwm_max)
   {
    config1 = 0;
    config2 = 1; 
    config3 = 0;
    config4 = 0;
    config5 = 0;
    config6 = 0;
    config7 = 0;
    config8 = 0;
    config9 = 0;
    config10 = 0;
    config11 = 0;
    config12 = 0;
    config13 = 0;
    config14 = 0;
    config15 = 0;
    config16 = 0;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 2 = ACTIVE");

   }
   else if (read_input_pwm_2 >= config3_pwm_min && read_input_pwm_2 <= config3_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 1;
    config4 = 0;
    config5 = 0;
    config6 = 0;
    config7 = 0;
    config8 = 0;
    config9 = 0;
    config10 = 0;
    config11 = 0;
    config12 = 0;
    config13 = 0;
    config14 = 0;
    config15 = 0;
    config16 = 0;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 3 = ACTIVE");
   }

   else if (read_input_pwm_2 >= config4_pwm_min && read_input_pwm_2 <= config4_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 0;
    config4 = 1;
    config5 = 0;
    config6 = 0;
    config7 = 0;
    config8 = 0;
    config9 = 0;
    config10 = 0;
    config11 = 0;
    config12 = 0;
    config13 = 0;
    config14 = 0;
    config15 = 0;
    config16 = 0;
    config17 = 0;
    
   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 4 = ACTIVE");
   }

   else if (read_input_pwm_2 >= config5_pwm_min && read_input_pwm_2 <= config5_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 0;
    config4 = 0;
    config5 = 1;
    config6 = 0;
    config7 = 0;
    config8 = 0;
    config9 = 0;
    config10 = 0;
    config11 = 0;
    config12 = 0;
    config13 = 0;
    config14 = 0;
    config15 = 0;
    config16 = 0;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 5 = ACTIVE");

   }
   else if (read_input_pwm_2 >= config6_pwm_min && read_input_pwm_2 <= config6_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 0;
    config4 = 0;
    config5 = 0;
    config6 = 1;
    config7 = 0;
    config8 = 0;
    config9 = 0;
    config10 = 0;
    config11 = 0;
    config12 = 0;
    config13 = 0;
    config14 = 0;
    config15 = 0;
    config16 = 0;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 6 = ACTIVE");

   }

   else if (read_input_pwm_2 >= config7_pwm_min && read_input_pwm_2 <= config7_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 0;
    config4 = 0;
    config5 = 0;
    config6 = 0;
    config7 = 1;
    config8 = 0;
    config9 = 0;
    config10 = 0;
    config11 = 0;
    config12 = 0;
    config13 = 0;
    config14 = 0;
    config15 = 0;
    config16 = 0;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 7 = ACTIVE");

   }

   else if (read_input_pwm_2 >= config8_pwm_min && read_input_pwm_2 <= config8_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 0;
    config4 = 0;
    config5 = 0;
    config6 = 0;
    config7 = 0;
    config8 = 1;
    config9 = 0;
    config10 = 0;
    config11 = 0;
    config12 = 0;
    config13 = 0;
    config14 = 0;
    config15 = 0;
    config16 = 0;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 8 = ACTIVE");

   }

   else if (read_input_pwm_2 >= config9_pwm_min && read_input_pwm_2 <= config9_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 0;
    config4 = 0;
    config5 = 0;
    config6 = 0;
    config7 = 0;
    config8 = 0;
    config9 = 1;
    config10 = 0;
    config11 = 0;
    config12 = 0;
    config13 = 0;
    config14 = 0;
    config15 = 0;
    config16 = 0;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 9 = ACTIVE");

   }

   else if (read_input_pwm_2 >= config10_pwm_min && read_input_pwm_2 <= config10_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 0;
    config4 = 0;
    config5 = 0;
    config6 = 0;
    config7 = 0;
    config8 = 0;
    config9 = 0;
    config10 = 1;
    config11 = 0;
    config12 = 0;
    config13 = 0;
    config14 = 0;
    config15 = 0;
    config16 = 0;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 10 = ACTIVE");

   }

   else if (read_input_pwm_2 >= config11_pwm_min && read_input_pwm_2 <= config11_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 0;
    config4 = 0;
    config5 = 0;
    config6 = 0;
    config7 = 0;
    config8 = 0;
    config9 = 0;
    config10 = 0;
    config11 = 1;
    config12 = 0;
    config13 = 0;
    config14 = 0;
    config15 = 0;
    config16 = 0;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 11 = ACTIVE");

   }

   else if (read_input_pwm_2 >= config12_pwm_min && read_input_pwm_2 <= config12_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 0;
    config4 = 0;
    config5 = 0;
    config6 = 0;
    config7 = 0;
    config8 = 0;
    config9 = 0;
    config10 = 0;
    config11 = 0;
    config12 = 1;
    config13 = 0;
    config14 = 0;
    config15 = 0;
    config16 = 0;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 12 = ACTIVE");

   }

   else if (read_input_pwm_2 >= config13_pwm_min && read_input_pwm_2 <= config13_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 0;
    config4 = 0;
    config5 = 0;
    config6 = 0;
    config7 = 0;
    config8 = 0;
    config9 = 0;
    config10 = 0;
    config11 = 0;
    config12 = 0;
    config13 = 1;
    config14 = 0;
    config15 = 0;
    config16 = 0;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 13 = ACTIVE");

   }

   else if (read_input_pwm_2 >= config14_pwm_min && read_input_pwm_2 <= config14_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 0;
    config4 = 0;
    config5 = 0;
    config6 = 0;
    config7 = 0;
    config8 = 0;
    config9 = 0;
    config10 = 0;
    config11 = 0;
    config12 = 0;
    config13 = 0;
    config14 = 1;
    config15 = 0;
    config16 = 0;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 14 = ACTIVE");

   }

   else if (read_input_pwm_2 >= config15_pwm_min && read_input_pwm_2 <= config15_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 0;
    config4 = 0;
    config5 = 0;
    config6 = 0;
    config7 = 0;
    config8 = 0;
    config9 = 0;
    config10 = 0;
    config11 = 0;
    config12 = 0;
    config13 = 0;
    config14 = 0;
    config15 = 1;
    config16 = 0;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 15 = ACTIVE");

   }

   else if (read_input_pwm_2 >= config16_pwm_min && read_input_pwm_2 <= config16_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 0;
    config4 = 0;
    config5 = 0;
    config6 = 0;
    config7 = 0;
    config8 = 0;
    config9 = 0;
    config10 = 0;
    config11 = 0;
    config12 = 0;
    config13 = 0;
    config14 = 0;
    config15 = 0;
    config16 = 1;
    config17 = 0;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 16 = ACTIVE");

   }

   else if (read_input_pwm_2 >= config17_pwm_min && read_input_pwm_2 <= config17_pwm_max)
   {
    config1 = 0;
    config2 = 0; 
    config3 = 0;
    config4 = 0;
    config5 = 0;
    config6 = 0;
    config7 = 0;
    config8 = 0;
    config9 = 0;
    config10 = 0;
    config11 = 0;
    config12 = 0;
    config13 = 0;
    config14 = 0;
    config15 = 0;
    config16 = 0;
    config17 = 1;

   dataFile.print(curr_time_ms);
   dataFile.println(" : CONFIG 17 = ACTIVE");
   }
   //------------------------------------------------------
   dataFile.close();
}

void check_seeder1()
{
  File dataFile = SD.open("datalog.txt", FILE_WRITE);
  //-------------------------- Seeder1 -----------------------
  if (config2 == 1 || config3 == 1 || config4 == 1 || config5 == 1 || config6 == 1 || config11 == 1 || config13 == 1|| config15 == 1 || config17 == 1)
  {
  for(int i = 1;i<=BlockMaxLimit;i++)
  {
        rmcs.Enable_Digital_Mode(n1_id, 0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder1 = ENABLED");

        leds[16] = CRGB(0,255,0); // Green 
        leds[17] = CRGB(0,255,0);
        leds[18] = CRGB(0,255,0);
        leds[19] = CRGB(0,255,0);

        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder1 LED Status = GREEN");


      rmcs.Speed(n1_id,speed);   
      int n1_feed_back = rmcs.Speed_Feedback(n1_id);

      dataFile.print(curr_time_ms);
      dataFile.print(" : Base Motor1 Feedback RPM = ");
      dataFile.println(n1_feed_back);


      if(n1_feed_back < speed)
      {
        FBSLL = n1_feed_back;
      }
      else if (n1_feed_back >= speed)
      {
        FBSUL = n1_feed_back;
      }
      if (FBSUL && FBSLL) 
      {
        CurrentRPM = ((FBSLL+FBSUL)/2.0)*GR;
        errorRPM = (speed*GR) - CurrentRPM;
        //  Serial.print("1cond1 = ");
        //  Serial.println(CurrentRPM);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder1 RPM (cond1) = ");
         dataFile.println(CurrentRPM);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder1 Error RPM (cond1) = ");
         dataFile.println(errorRPM);
      }
      else if (FBSLL == 0 && n1_feed_back == 0)
      {
        CurrentRPM = 0 ;
        errorRPM = (speed*GR) - CurrentRPM;
        //  Serial.print("1cond2 = ");
        //  Serial.println(CurrentRPM);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder1 RPM (cond2) = ");
         dataFile.println(CurrentRPM);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder1 Error RPM (cond2) = ");
         dataFile.println(errorRPM);

      }
      else if (FBSLL)
      {
        CurrentRPM = FBSLL*GR;
        errorRPM = (speed*GR) - CurrentRPM;
        //  Serial.print("1cond3 = ");
        //  Serial.println(CurrentRPM);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder1 RPM (cond3) = ");
         dataFile.println(CurrentRPM);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder1 Error RPM (cond3) = ");
         dataFile.println(errorRPM);
      }
      else if (FBSUL)
      { 
        CurrentRPM = FBSUL*GR;
        errorRPM = (speed*GR) - CurrentRPM;
        //  Serial.print("1cond4 = ");
        //  Serial.println(CurrentRPM);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder1 RPM (cond4) = ");
         dataFile.println(CurrentRPM);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder1 Error RPM (cond4) = ");
         dataFile.println(errorRPM);
      }

      //-------------Seeder 1--------------------------------------
      if (CurrentRPM == 0)
      {
        BlockageCounter++;
      }
      if(BlockageCounter >= BlockMaxLimit)
      {
        dataFile.print(curr_time_ms);
        dataFile.println(" : * * * * * * * * * Seeder1 BLOCKED * * * * * * * * * ");
        
        rmcs.Disable_Digital_Mode(n1_id,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder1 = DISABLED");

        leds[19] = CRGB(255,0,0); //RED
        leds[18] = CRGB(255,0,0);
        leds[17] = CRGB(255,0,0);
        leds[16] = CRGB(255,0,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder1 LED Status = RED");

        // Serial.println("1Seed Blocked");
        digitalWrite(relayPin,HIGH);
        dataFile.print(curr_time_ms);
        dataFile.println(": Buzzer ON");

      } 
    //---------------------------------------------------------------
  }
  }
  else 
  {
        leds[19] = CRGB(255,0,255); //purple
        leds[18] = CRGB(255,0,255);
        leds[17] = CRGB(255,0,255);
        leds[16] = CRGB(255,0,255);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder1 LED Status = PURPLE");

        rmcs.Disable_Digital_Mode(n1_id,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder1 = DISABLED");
  }
    //--------------------------------------------------------------------
    dataFile.close();
}

void check_seeder2()
{
  File dataFile = SD.open("datalog.txt", FILE_WRITE);
     // ---------------------- Seeder 2 -----------------------------------
    if (config2 == 1 || config3 == 1 || config4 == 1 || config5 == 1 || config7 == 1 || config12 == 1 || config16 == 1)
  {
        rmcs.Enable_Digital_Mode(n2_id, 0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder2 = ENABLED");

        leds[12] = CRGB(0,255,0); // Green 
        leds[13] = CRGB(0,255,0);
        leds[14] = CRGB(0,255,0);
        leds[15] = CRGB(0,255,0);

        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder2 LED Status = GREEN");

      rmcs.Speed(n2_id,speed);   
      int n2_feed_back = rmcs.Speed_Feedback(n2_id);

      dataFile.print(curr_time_ms);
      dataFile.print(" : Base Motor2 Feedback RPM = ");
      dataFile.println(n2_feed_back);

      if(n2_feed_back < speed)
      {
        FBSLL2 = n2_feed_back;
      }
      else if (n2_feed_back >= speed)
      {
        FBSUL2 = n2_feed_back;
      }
      if (FBSUL2 && FBSLL2) 
      {
        CurrentRPM2 = ((FBSLL2+FBSUL2)/2.0)*GR;
        errorRPM2 = (speed*GR) - CurrentRPM2;
        //  Serial.print("2cond1 = ");
        //  Serial.println(CurrentRPM2);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder2 RPM (cond1) = ");
         dataFile.println(CurrentRPM2);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder2 Error RPM (cond1) = ");
         dataFile.println(errorRPM2);
      }
      else if (FBSLL2 == 0 && n2_feed_back == 0)
      {
        CurrentRPM2 = 0 ;
        errorRPM2 = (speed*GR) - CurrentRPM2;
        //  Serial.print("2cond2 = ");
        //  Serial.println(CurrentRPM2);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder2 RPM (cond2) = ");
         dataFile.println(CurrentRPM2);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder2 Error RPM (cond2) = ");
         dataFile.println(errorRPM2);
      }
      else if (FBSLL2)
      {
        CurrentRPM2 = FBSLL2*GR;
        errorRPM2 = (speed*GR) - CurrentRPM2;
        //  Serial.print("2cond3 = ");
        //  Serial.println(CurrentRPM2);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder2 RPM (cond3) = ");
         dataFile.println(CurrentRPM2);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder2 Error RPM (cond3) = ");
         dataFile.println(errorRPM2);
      }
      else if (FBSUL2)
      { 
        CurrentRPM2 = FBSUL2*GR;
        errorRPM2 = (speed*GR) - CurrentRPM2;
        //  Serial.print("2cond4 = ");
        //  Serial.println(CurrentRPM2);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder2 RPM (cond4) = ");
         dataFile.println(CurrentRPM2);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder2 Error RPM (cond4) = ");
         dataFile.println(errorRPM2);
      }
      //-------------------Seeder 2------------------------------------
     if (CurrentRPM2 == 0)
      {
        BlockageCounter2++;
      }
      if(BlockageCounter2 >= BlockMaxLimit)
      {
        dataFile.print(curr_time_ms);
        dataFile.println(" : * * * * * * * * * Seeder2 BLOCKED * * * * * * * * * ");
        
        rmcs.Disable_Digital_Mode(n2_id,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder2 = DISABLED");

        leds[15] = CRGB(255,0,0); //RED
        leds[14] = CRGB(255,0,0);
        leds[13] = CRGB(255,0,0);
        leds[12] = CRGB(255,0,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder2 LED Status = RED");


        // Serial.println("2Seed Blocked");
        digitalWrite(relayPin,HIGH);
        dataFile.print(curr_time_ms);
        dataFile.println(": Buzzer ON");

      } 
    //--------------------------------------------------------------
  }
    else 
  {
        leds[15] = CRGB(255,0,255); //purple
        leds[14] = CRGB(255,0,255);
        leds[13] = CRGB(255,0,255);
        leds[12] = CRGB(255,0,255);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder2 LED Status = PURPLE");

        rmcs.Disable_Digital_Mode(n2_id,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder2 = DISABLED");
  }
//--------------------------------------------------------------
  dataFile.close();
}

void check_seeder3()
{
  File dataFile = SD.open("datalog.txt", FILE_WRITE);
  // ---------------------- Seeder 3 -----------------------------------
if (config2 == 1 || config3 == 1 || config4 == 1 || config7 == 1 || config8 == 1 || config11 == 1|| config13 == 1 || config14 == 1)
  {
      rmcs.Enable_Digital_Mode(n3_id, 0);
      dataFile.print(curr_time_ms);
      dataFile.println(" : Seeder3 = ENABLED");

        leds[8] = CRGB(0,255,0); // Green 
        leds[9] = CRGB(0,255,0);
        leds[10] = CRGB(0,255,0);
        leds[11] = CRGB(0,255,0);

        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder3 LED Status = GREEN");

      rmcs.Speed(n3_id,speed);   
      int n3_feed_back = rmcs.Speed_Feedback(n3_id);

      dataFile.print(curr_time_ms);
      dataFile.print(" : Base Motor3 Feedback RPM = ");
      dataFile.println(n3_feed_back);

      if(n3_feed_back < speed)
      {
        FBSLL3 = n3_feed_back;
      }
      else if (n3_feed_back >= speed)
      {
        FBSUL3 = n3_feed_back;
      }
      if (FBSUL3 && FBSLL3) 
      {
        CurrentRPM3 = ((FBSLL3+FBSUL3)/2.0)*GR;
        errorRPM3 = (speed*GR) - CurrentRPM3;
        //  Serial.print("3cond1 = ");
        //  Serial.println(CurrentRPM3);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder3 RPM (cond1) = ");
         dataFile.println(CurrentRPM3);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder3 Error RPM (cond1) = ");
         dataFile.println(errorRPM3);
      }
      else if (FBSLL3 == 0 && n3_feed_back == 0)
      {
        CurrentRPM3 = 0 ;
        errorRPM3 = (speed*GR) - CurrentRPM3;
        //  Serial.print("3cond2 = ");
        //  Serial.println(CurrentRPM3);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder3 RPM (cond2) = ");
         dataFile.println(CurrentRPM3);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder3 Error RPM (cond2) = ");
         dataFile.println(errorRPM3);
      }
      else if (FBSLL3)
      {
        CurrentRPM3 = FBSLL3*GR;
        errorRPM3 = (speed*GR) - CurrentRPM3;
        //  Serial.print("3cond3 = ");
        //  Serial.println(CurrentRPM3);
        dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder3 RPM (cond3) = ");
         dataFile.println(CurrentRPM3);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder3 Error RPM (cond3) = ");
         dataFile.println(errorRPM3);
      }
      else if (FBSUL3)
      { 
        CurrentRPM3 = FBSUL3*GR;
        errorRPM3 = (speed*GR) - CurrentRPM3;
        //  Serial.print("3cond4 = ");
        //  Serial.println(CurrentRPM3);
        dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder3 RPM (cond4) = ");
         dataFile.println(CurrentRPM3);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder3 Error RPM (cond4) = ");
         dataFile.println(errorRPM3);
      }

      //-------------------Seeder 3------------------------------------
     if (CurrentRPM3 == 0)
      {
        BlockageCounter3++;
      }
      if(BlockageCounter3 >= BlockMaxLimit)
      {
        dataFile.print(curr_time_ms);
        dataFile.println(" : * * * * * * * * * Seeder3 BLOCKED * * * * * * * * * ");
        
        rmcs.Disable_Digital_Mode(n3_id,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder3 = DISABLED");

        leds[11] = CRGB(255,0,0); //RED
        leds[10] = CRGB(255,0,0);
        leds[9] = CRGB(255,0,0);
        leds[8] = CRGB(255,0,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder3 LED Status = RED");


        // Serial.println("3Seed Blocked");
        digitalWrite(relayPin,HIGH);
        dataFile.print(curr_time_ms);
        dataFile.println(": Buzzer ON");
      } 
    //----------------------------------------------------------------
   }
    else 
  {
        leds[11] = CRGB(255,0,255); //purple
        leds[10] = CRGB(255,0,255);
        leds[9] = CRGB(255,0,255);
        leds[8] = CRGB(255,0,255);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder3 LED Status = PURPLE");

        rmcs.Disable_Digital_Mode(n3_id,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder3 = DISABLED");
  }
    //--------------------------------------------------------------
    dataFile.close();
}

void check_seeder4()
{
  File dataFile = SD.open("datalog.txt", FILE_WRITE);
  // ---------------------- Seeder 4 -----------------------------------
if (config2 == 1 || config3 == 1 || config7 == 1 || config8 == 1 || config9 == 1 || config12 == 1 || config15 == 1)
  {
      rmcs.Enable_Digital_Mode(n4_id, 0);
      dataFile.print(curr_time_ms);
      dataFile.println(" : Seeder4 = ENABLED");

        leds[4] = CRGB(0,255,0); // Green 
        leds[5] = CRGB(0,255,0);
        leds[6] = CRGB(0,255,0);
        leds[7] = CRGB(0,255,0);

        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder4 LED Status = GREEN");

      rmcs.Speed(n4_id,speed);   
      int n4_feed_back = rmcs.Speed_Feedback(n4_id);

     dataFile.print(curr_time_ms);
     dataFile.print(" : Base Motor4 Feedback RPM = ");
     dataFile.println(n4_feed_back);

      if(n4_feed_back < speed)
      {
        FBSLL4 = n4_feed_back;
      }
      else if (n4_feed_back >= speed)
      {
        FBSUL4 = n4_feed_back;
      }
      if (FBSUL4 && FBSLL4) 
      {
        CurrentRPM4 = ((FBSLL4+FBSUL4)/2.0)*GR;
        errorRPM4 = (speed*GR) - CurrentRPM4;
        //  Serial.print("4cond1 = ");
        //  Serial.println(CurrentRPM4);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder4 RPM (cond1) = ");
         dataFile.println(CurrentRPM4);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder4 Error RPM (cond1) = ");
         dataFile.println(errorRPM4);

      }
      else if (FBSLL4 == 0 && n4_feed_back == 0)
      {
        CurrentRPM4 = 0 ;
        errorRPM4 = (speed*GR) - CurrentRPM4;
        // Serial.print("4cond2 = ");
        //  Serial.println(CurrentRPM4);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder4 RPM (cond2) = ");
         dataFile.println(CurrentRPM4);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder4 Error RPM (cond2) = ");
         dataFile.println(errorRPM4);
      }
      else if (FBSLL4)
      {
        CurrentRPM4 = FBSLL4*GR;
        errorRPM4 = (speed*GR) - CurrentRPM4;
        // Serial.print("4cond3 = ");
        //  Serial.println(CurrentRPM4);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder4 RPM (cond3) = ");
         dataFile.println(CurrentRPM4);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder4 Error RPM (cond3) = ");
         dataFile.println(errorRPM4);
      }
      else if (FBSUL4)
      { 
        CurrentRPM4 = FBSUL4*GR;
        errorRPM4 = (speed*GR) - CurrentRPM4;
        // Serial.print("4cond4 = ");
        //  Serial.println(CurrentRPM4);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder4 RPM (cond4) = ");
         dataFile.println(CurrentRPM4);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder4 Error RPM (cond4) = ");
         dataFile.println(errorRPM4);
      }
      //-------------------Seeder 4------------------------------------
     if (CurrentRPM4 == 0)
      {
        BlockageCounter4++;
      }
      if(BlockageCounter4 >= BlockMaxLimit)
      {
        dataFile.print(curr_time_ms);
        dataFile.println(" : * * * * * * * * * Seeder4 BLOCKED * * * * * * * * * ");
        
        rmcs.Disable_Digital_Mode(n4_id,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder4 = DISABLED");

        leds[7] = CRGB(255,0,0); //RED
        leds[6] = CRGB(255,0,0);
        leds[5] = CRGB(255,0,0);
        leds[4] = CRGB(255,0,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder4 LED Status = RED");

        // Serial.println("4Seed Blocked");
        digitalWrite(relayPin,HIGH);
        dataFile.print(curr_time_ms);
        dataFile.println(": Buzzer ON");

      } 
    //----------------------------------------------------------------

  }
  else 
  {
        leds[7] = CRGB(255,0,255); //purple
        leds[6] = CRGB(255,0,255);
        leds[5] = CRGB(255,0,255);
        leds[4] = CRGB(255,0,255);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder4 LED Status = PURPLE");

        rmcs.Disable_Digital_Mode(n4_id,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder4 = DISABLED");
  }
    //--------------------------------------------------------------
    dataFile.close();
}

void check_seeder5()
{
  File dataFile = SD.open("datalog.txt", FILE_WRITE);
  // ------------------ Seeder 5 -----------------------------------
if (config2 == 1 || config7 == 1 || config8 == 1 || config9 == 1 || config10 == 1 || config11 == 1 || config14 == 1|| config16 == 1 || config17 == 1)
  {
      rmcs.Enable_Digital_Mode(n5_id, 0);
      dataFile.print(curr_time_ms);
      dataFile.println(" : Seeder5 = ENABLED");
      

        leds[0] = CRGB(0,255,0); // Green 
        leds[1] = CRGB(0,255,0);
        leds[2] = CRGB(0,255,0);
        leds[3] = CRGB(0,255,0);
        
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder5 LED Status = GREEN");

      rmcs.Speed(n5_id,speed);   
     n5_feed_back = rmcs.Speed_Feedback(n5_id);

     dataFile.print(curr_time_ms);
     dataFile.print(" : Base Motor5 Feedback RPM = ");
     dataFile.println(n5_feed_back);

      if(n5_feed_back < speed)
      {
        FBSLL5 = n5_feed_back;
      }
      else if (n5_feed_back >= speed)
      {
        FBSUL5 = n5_feed_back;
      }
      if (FBSUL5 && FBSLL5) 
      {
         CurrentRPM5 = ((FBSLL5+FBSUL5)/2.0)*GR;
         errorRPM5 = (speed*GR) - CurrentRPM5;

        //  Serial.print("5cond1 = ");
        //  Serial.println(CurrentRPM5);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder5 RPM (cond1) = ");
         dataFile.println(CurrentRPM5);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder5 Error RPM (cond1) = ");
         dataFile.println(errorRPM5);

      }
      else if (FBSLL5 == 0 && n5_feed_back == 0)
      {
        CurrentRPM5 = 0 ;
        errorRPM5 = (speed*GR) - CurrentRPM5;

        Serial.print("5cond2 = ");
         Serial.println(CurrentRPM5);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder5 RPM (cond2) = ");
         dataFile.println(CurrentRPM5);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder5 Error RPM (cond2) = ");
         dataFile.println(errorRPM5);
      }
      else if (FBSLL5)
      {
        CurrentRPM5 = FBSLL5*GR;
        errorRPM5 = (speed*GR) - CurrentRPM5;

        Serial.print("5cond3 = ");
         Serial.println(CurrentRPM5);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder5 RPM (cond3) = ");
         dataFile.println(CurrentRPM5);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder5 Error RPM (cond3) = ");
         dataFile.println(errorRPM5);
      }
      else if (FBSUL5)
      { 
        CurrentRPM5 = FBSUL5*GR;
        errorRPM5 = (speed*GR) - CurrentRPM5;

        Serial.print("5cond4 = ");
         Serial.println(CurrentRPM5);
         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder5 RPM (cond4) = ");
         dataFile.println(CurrentRPM5);

         dataFile.print(curr_time_ms);
         dataFile.print(" : Seeder5 Error RPM (cond4) = ");
         dataFile.println(errorRPM5);
      }
    //--------------------------------------------------------------
    //-------------------Seeder 5------------------------------------
     if (CurrentRPM5 == 0)
      {
        BlockageCounter5++;
      }
      if(BlockageCounter5 >= BlockMaxLimit)
      {
        Serial.println("5Seed Blocked");

        dataFile.print(curr_time_ms);
        dataFile.println(" : * * * * * * * * * Seeder5 BLOCKED * * * * * * * * * ");

        rmcs.Disable_Digital_Mode(n5_id,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder5 = DISABLED");
        
        leds[3] = CRGB(255,0,0); //RED
        leds[2] = CRGB(255,0,0);
        leds[1] = CRGB(255,0,0);
        leds[0] = CRGB(255,0,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder5 LED Status = RED");

        digitalWrite(relayPin,HIGH);
        dataFile.print(curr_time_ms);
        dataFile.println(": Buzzer ON");
      } 
    
  }
 else 
  {
        leds[3] = CRGB(255,0,255); //purple
        leds[2] = CRGB(255,0,255);
        leds[1] = CRGB(255,0,255);
        leds[0] = CRGB(255,0,255);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder5 LED Status = PURPLE");

        rmcs.Disable_Digital_Mode(n5_id,0);
        dataFile.print(curr_time_ms);
        dataFile.println(" : Seeder5 = DISABLE");
  }

//----------------------------------------------------------------
dataFile.close();

}

void check_relay()
{
   File dataFile = SD.open("datalog.txt", FILE_WRITE);
  //-------------- Relay Low ------------------
    if ((BlockageCounter < BlockMaxLimit) && (BlockageCounter2 < BlockMaxLimit) && (BlockageCounter3 < BlockMaxLimit) && (BlockageCounter4 < BlockMaxLimit) && (BlockageCounter5 < BlockMaxLimit ))
    {
      digitalWrite(relayPin,LOW);
      dataFile.println("Buzzer OFF");
    }
    dataFile.close();
}

void check_rpm_cutoff()
{
  File dataFile = SD.open("datalog.txt", FILE_WRITE);
   if(speed >= rpmCutOff)
 {
   BlockageCounter = 0;
   BlockageCounter2 = 0;
   BlockageCounter3 = 0;
   BlockageCounter4 = 0;
   BlockageCounter5 = 0;

   for(int i = 1;i<=BlockMaxLimit;i++)
  {
   
  curr_time_ms = millis(); 
  // check_seeder1();
  // check_seeder2();
  // check_seeder3();
  // check_seeder4();
   check_seeder5();
   delay(dt); 
  }
   //check_relay();
 }
 else 
 {
  //  rmcs.Disable_Digital_Mode(n1_id,0);
  //  rmcs.Disable_Digital_Mode(n2_id,0);
  //  rmcs.Disable_Digital_Mode(n3_id,0);
  //  rmcs.Disable_Digital_Mode(n4_id,0);
    rmcs.Disable_Digital_Mode(n5_id,0);
    dataFile.print(curr_time_ms);
    dataFile.println(" : All Motors = DISABLED");

    // --------------- All Turned OFF ------------

        leds[3] = CRGB(0,150,255); //blue
        leds[2] = CRGB(0,150,255);
        leds[1] = CRGB(0,150,255);
        leds[0] = CRGB(0,150,255);

        // leds[7] = CRGB(0,150,255); //blue
        // leds[6] = CRGB(0,150,255);
        // leds[5] = CRGB(0,150,255);
        // leds[4] = CRGB(0,150,255);

        // leds[11] = CRGB(0,150,255); //blue
        // leds[10] = CRGB(0,150,255);
        // leds[9] = CRGB(0,150,255);
        // leds[8] = CRGB(0,150,255);

        // leds[15] = CRGB(0,150,255); //blue
        // leds[14] = CRGB(0,150,255);
        // leds[13] = CRGB(0,150,255);
        // leds[12] = CRGB(0,150,255);

        // leds[19] = CRGB(0,150,255); //blue
        // leds[18] = CRGB(0,150,255);
        // leds[17] = CRGB(0,150,255);
        // leds[16] = CRGB(0,150,255);
    //-----------------------------------------------------

 }
 FastLED.show();
 dataFile.close();
}

void loop()
{
   //1) Speed Select
   //2) Configuration Select 
   //3) Check RPM Cutoff
   curr_time_ms = millis();
   speed_select();
   configuration_select();
   check_rpm_cutoff();


}

