...
int max_rpm = 66; //**********  NEED TO CHECK *****************(JGB 37 - 520 - 12V - 66RPM (66 RPM)) 
...
void seed_block_function(int speed)
{
  BlockageCounter = 0;
  for(int i = 1;i<=BlockMaxLimit;i++)
  {
    for(int j = 1;j<=5;j++)
    {
     // rmcs.Enable_Digital_Mode(n1_id, 0);
      rmcs.Enable_Digital_Mode(j, 0);
      rmcs.Speed(n1_id,speed);   
      //int n1_feed_back = rmcs.Speed_Feedback(n1_id);
      int n1_feed_back = rmcs.Speed_Feedback(j);

      if((n1_feed_back < speed) && j == 1)
      {
        FBSLL = n1_feed_back;
      }
      else if ((n1_feed_back >= speed) && j ==1)
      {
        FBSUL = n1_feed_back;
      }
